...
class FinQANoNumbersRetrieval(AbsTaskRetrieval):
    metadata = TaskMetadata(
        name="FinQANoNumbersRetrieval",
        description=(
            "FinQA retrieval with corpus numeric tokens stripped. Corpus passages come from FinQA after "
            "removing standalone numeric values; queries/qrels remain original."
        ),
        ...
        prompt={
            "query": "Given a financial question, retrieve supporting evidence with numbers removed from the corpus."
        },
    )
    ...
    def load_data(self) -> None:
        ...
        corpus_path = _local_corpus_dir("finqa_no_numbers")
