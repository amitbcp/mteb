...
def get_generative_ai_dp_client(endpoint, profile, use_session_token):
    ...
    if use_session_token:
        ...
        return oci.generative_ai_inference.GenerativeAiInferenceClient(
            config=config, signer=signer, service_endpoint=endpoint, timeout=(10, 240)
        )
    else:
        return oci.generative_ai_inference.GenerativeAiInferenceClient(
            config=config, service_endpoint=endpoint, timeout=(10, 240)
        )
...
def checkCompartmentPresent(compartment_id):
    if "<compartment_ocid>" in compartment_id:
        print(
            "ERROR:Please update your compartment id via env variable GENAI_COMPARMENT"
        )
        quit()
...
def get_inference(infer_image_url):
    generative_ai_inference_client = get_generative_ai_dp_client(
        endpoint=getEndpoint(region, stage), profile=profile, use_session_token=True
    )
    ...
    chat_detail.serving_mode = oci.generative_ai_inference.models.DedicatedServingMode(
        endpoint_id="ocid1.generativeaiendpoint.oc1.us-chicago-1.amaaaaaabgjpxjqa2n7d2sfaiyjvgws6tqbkzy3i754hipxf7cojrvk6aqaa"
    )
...
def get_inference_gpt4mini(infer_image_url):
    ...
    # Here to update to use desired model_id, more instruction in above TODO 2
    chat_detail.serving_mode = oci.generative_ai_inference.models.OnDemandServingMode(
        model_id="ocid1.generativeaimodel.oc1.us-chicago-1.amaaaaaask7dceyacg3e2j2kkruumwqpiewrrng2cakvncpmrlfoxsjemfqa"
    )
    chat_detail.compartment_id = compartment_id
    ...
    if model == "gpt4o":
        ...
        for i in range(count):
            try:
                result = get_inference_ocigenai(
                    image_url,
                    model_id="ocid1.generativeaimodel.oc1.us-chicago-1.amaaaaaask7dceya7kvy7cgf7ysbmk3slp2b7qdphkgfjfvh2wcbw3rhk3iq",
                )
                break
            except oci.exceptions.ServiceError as e:
                if e.status == 413:
...
def run_parallel(image_list: List[str], max_threads=20, model="llama4-17b-16e-scout"):
    ...
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        futures = [
            executor.submit(worker, idx, url, model)
            for idx, url in enumerate(image_list)
        ]
        for future in tqdm(
            as_completed(futures),
            total=len(futures),
            desc="OCI Generative AI Inference",
        ):
...
def run_parallel_progress(
    image_list: List[str], max_threads=10, model="llama4-17b-16e-scout"
):
    results = [None] * len(image_list)  # Pre-allocate result list
    print("Running OCI Generative AI Inference in parallel...")
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        ...
        for future in tqdm(
            as_completed(futures),
            total=len(futures),
            desc="OCI Generative AI Inference last remaning data",
        ):
            # for future in as_completed(futures):
            index, result = future.result()
            results[index] = result  # Store in correct order
