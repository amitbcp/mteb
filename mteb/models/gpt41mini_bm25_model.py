                ...
                for batch in images:
                    for image_tensor in batch:
                        ...
                        if len(image_list) >= 10000:
                            # pdb.set_trace()
                            image_texts = run_parallel(image_list,max_threads=15,model="gpt41mini")
                            all_image_texts.extend(image_texts)
                            image_list = []
                # for the remaining images
                if len(image_list) > 0:
                    image_texts = run_parallel_progress(image_list,max_threads=15,model="gpt41mini")
                    all_image_texts.extend(image_texts)
...
gpt41mini_bm25 = ModelMeta(
    loader=partial(
        GPT41MiniBM25Wrapper,
