...
def create_slurm_job_file(
    ...
    slurm_job += f"python benchmark_task.py --model_name  {model_name} --task_name  {task_name} --batch_size {batch_size}"

    model_path_name = model_name.replace("/", "__")

    slurm_job_file = slurm_jobs_folder / f"{model_path_name}_{task_type}.sh"
...
if __name__ == "__main__":
    ...
    files = create_slurm_job_files(
        args.model_names, task_names, results_folder, slurm_prefix, slurm_jobs_folder, args.device
    )
